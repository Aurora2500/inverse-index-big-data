...
public class BookFilter {
    ...
    public void filter() {
        extractedDatalakeInformation();

    }

    private static void extractedDatalakeInformation() {
        String datalakeDir = "C:/Users/Jaime/Desktop/Datalake/";
        File datalake = new File(datalakeDir);
        File[] folders = datalake.listFiles();

        try {
            extractedFolders(folders, datalakeDir);
        } catch (Exception e) {
            System.out.println("Error: " + e.getMessage());
        }
    }

    private static void extractedFolders(File[] folders, String datalakeDir) throws IOException {
        if (folders != null) {
            ...
            for (File folder : folders) {
                ...
                if (books != null) {
                    getJsonFile(books, newDatalakeDir);
    ...
    private static void getJsonFile(File[] books, String newDatalakeDir) throws IOException {
        for (File book : books) {
            ...
            if (fileName.endsWith(".json")) {
                ...
                Book content = getBookParameters(rootNode, date, author, text, title, lang, docId);
    ...
    private static Book getBookParameters(JsonNode rootNode, String date, String author, String text, String title, String lang, int docId) {
        ...
        return new Book(docId, date, author, text, title, lang);
    }
